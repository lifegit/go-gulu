...
func (c *ColumnSchema) ToProperty(authTable, passwordColumn string) Property {
	...
	// [State,Tag,Remarks|...]
	var tags []ColumnCommentTag
	if start, end := strings.Index(c.ColumnComment, "["), strings.Index(c.ColumnComment, "]"); start != -1 && end != -1 {
		item := strings.Split(c.ColumnComment[start+1:end], "|")
		for _, value := range item {
			if item2 := strings.Split(value, ","); len(item2) >= 3 {
				tags = append(tags,
					ColumnCommentTag{
						State:   item2[0],
						Tag:     strcase.ToCamel(item2[1]),
						Remarks: item2[2],
					})
			}
		}
	}
	p.ColumnCommentTag = tags
