...
func TestCrudCreate(t *testing.T) {
	...
	assert.Equal(t, DBDryRun.Logger.(*fire.Diary).LastSql(), "INSERT INTO `user` (`company_id`,`name`,`tag`,`age`,`height`) VALUES (1,'san','student',18,185)")

	_ = DBDryRun.CrudCreate(&[]User{
		{CompanyID: 1, Name: "san", Tag: "student", Height: 185},
		{CompanyID: 2, Name: "sid", Tag: "student", Age: 19},
	}, 2)
	assert.Equal(t, DBDryRun.Logger.(*fire.Diary).LastSql(), "INSERT INTO `user` (`company_id`,`name`,`tag`,`age`,`height`) VALUES (1,'san','student',0,185),(2,'sid','student',19,0)")
}

func TestIsExists(t *testing.T) {
	DBDryRun.IsExists(User{ID: 1, Name: "Mr Wang"})
	assert.Equal(t, DBDryRun.Logger.(*fire.Diary).LastSql(), "SELECT 1 FROM `user` WHERE `user`.`id` = 1 AND `user`.`name` = 'Mr Wang' LIMIT 1")
}

func TestCrudOne(t *testing.T) {
	_ = DBDryRun.WhereLike("name", "M").CrudOne(User{ID: 1, Age: 20}, &User{})
	assert.Equal(t, DBDryRun.Logger.(*fire.Diary).LastSql(), "SELECT * FROM `user` WHERE `name` LIKE '%M%' AND `user`.`id` = 1 AND `user`.`age` = 20 LIMIT 1")
}

func TestCrudAll(t *testing.T) {
	_ = DBDryRun.WhereRange("age", 18, 20).CrudAll(User{}, &[]User{})
	assert.Equal(t, DBDryRun.Logger.(*fire.Diary).LastSql(), "SELECT * FROM `user` WHERE `age` >= 18 AND `age` <= 20")
}
...
func TestCrudCount(t *testing.T) {
	...
	assert.Equal(t, DBDryRun.Logger.(*fire.Diary).LastSql(), "SELECT count(*) FROM `user` WHERE `age` >= 18 AND `age` <= 20")
}

func TestCrudSum(t *testing.T) {
	_, _ = DBDryRun.WhereRange("age", 18, 20).CrudSum(User{}, "id")
	assert.Equal(t, DBDryRun.Logger.(*fire.Diary).LastSql(), "SELECT IFNULL(SUM(`id`),0) FROM `user` WHERE `age` >= 18 AND `age` <= 20")
}
...
func TestCrudUpdatePrimaryKey(t *testing.T) {
	...
	assert.Equal(t, DBDryRun.Logger.(*fire.Diary).LastSql(), "UPDATE `user` SET `name`='LI' WHERE `user`.`id` = 1 AND `id` = 1 LIMIT 1")
}

func TestCrudDelete(t *testing.T) {
	_ = DBDryRun.WhereRange("age", 18, 20).CrudDelete(&User{})
	assert.Equal(t, DBDryRun.Logger.(*fire.Diary).LastSql(), "DELETE FROM `user` WHERE `age` >= 18 AND `age` <= 20")
}
