...
func TestCreatedAt(t *testing.T) {
	...
	sql := fmt.Sprintf(`INSERT INTO "company" ("created_at","updated_at","address","name") VALUES (%d,%d,'Shanghai','lu') RETURNING "id"`, tx.Statement.Vars[0], tx.Statement.Vars[1])
...
func TestUpdatedAt(t *testing.T) {
	...
	sql := fmt.Sprintf(`UPDATE "company" SET "updated_at"=%d,"address"='Jinan' WHERE "company"."address" = 'Shanghai' AND "company"."deleted_at" = 0`, tx.Statement.Vars[0])
	assert.Equal(t, DBDryRun.Logger.(*fire.Diary).LastSql(), sql)
}
