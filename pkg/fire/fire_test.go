...
func TestFormatColumn(t *testing.T) {
	...
	// table column all
	success = "`table`.*"
	res = fire.FormatColumn(fmt.Sprintf("table.%s", fire.ColumnAll))
	assert.Equal(t, res, success)
	res = fire.FormatColumn("table", fire.ColumnAll)
	assert.Equal(t, res, success)
...
func TestIn(t *testing.T) {
	...
	assert.Equal(t, DBDryRun.Logger.(*fire.Diary).LastSql(), "SELECT * FROM `user` WHERE `age`  IN (18,19,20) LIMIT 1")

	DBDryRun.WhereIn("age", []int{18, 19, 20}, true).Model(User{}).Take(&User{})
	assert.Equal(t, DBDryRun.Logger.(*fire.Diary).LastSql(), "SELECT * FROM `user` WHERE `age` NOT IN (18,19,20) LIMIT 1")
}

func TestLike(t *testing.T) {
	DBDryRun.WhereLike("name", "Wang").Model(User{}).Take(&User{})
	assert.Equal(t, DBDryRun.Logger.(*fire.Diary).LastSql(), "SELECT * FROM `user` WHERE `name` LIKE '%Wang%' LIMIT 1")
}

func TestRange(t *testing.T) {
	DBDryRun.WhereRange("age", 10, 20).Model(User{}).Take(&User{})
	assert.Equal(t, DBDryRun.Logger.(*fire.Diary).LastSql(), "SELECT * FROM `user` WHERE `age` >= 10 AND `age` <= 20 LIMIT 1")
}
...
func TestOrderByColumn(t *testing.T) {
	...
	assert.Equal(t, DBDryRun.Logger.(*fire.Diary).LastSql(), "SELECT * FROM `user` ORDER BY `age` asc")
}
